function out1 = mst_nochecks(A,varargin)
              % mst_nochecks(cost)
...
options = struct('algname', 'prim', 'edge_weight', 'matrix', ...
    'root', 'none', 'fix_diag', 1);                   %四个属性 options.algname, options.edge_weight, options.root, options.fix_diag
                                                      % 对应的值为'prim', 'matrix', 'none', 1
...
[i j v] = mst_mex(A,lower(options.algname),edge_weight_opt,0); % options.algname为'prim'
                                                               % edge_weight_opt为'matrix'
                                                               % lower函数 将字母变为小写
                                                               % mst_mex C++代码
                                                               % C++

T = sparse(i,j,v,size(A,1),size(A,1));  % 依次取i,j中的值作为T的行标和列标，再在v中的值赋值到T中所对应的行标和列标 
T = T + T';
out1 = T;
