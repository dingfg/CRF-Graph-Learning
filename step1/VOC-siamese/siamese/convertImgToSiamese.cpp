...
DEFINE_int32(resize_width, 28, "Width images are resized to");
DEFINE_int32(resize_height, 28, "Height images are resized to");
...
int main(int argc, char** argv)
{
    ...
    char* Pixels = new char[2 * 28 * 28 * 3];    //6. add channel
    ...
    datum.set_channels(6);                //7. 3 channels for each image in the pair
    datum.set_height(28);
    datum.set_width(28);
    ...
    for (int LineIndex = 0; LineIndex < lines.size(); LineIndex++)
    {
        ...
        ReadImageToMemory(root_folder + lines[LineIndex].first, 28, 28, FirstImagePixel);  //8. add channel here

        //char *SecondImagePixel = Pixels + resize_width * resize_height;
        char *SecondImagePixel = Pixels + 28 * 28 * 3;       //10. add channel
        ReadImageToMemory(root_folder + lines[LineIndex].second, 28, 28, SecondImagePixel);  //9. add channel here

        // set image pair data
        // datum.set_data(Pixels, 2 * resize_height * resize_width);
        datum.set_data(Pixels, 2 * 28 * 28 * 3);     //11. correct
	...
	if (label_list[LineIndex]=="1")
        ...
        //cout << "label: " << datum.label() << ' ' << "key index: " << key_str << endl;
